...
const projectId = route.params.projectName;
...
onMounted(async () => {
    await commentStore.getComments(projectId);
    commentStore.listenComments(projectId);
    scrollToBottom();
});

onUnmounted(() => {
    commentStore.unlistenComments(projectId);
});
...
const onFileChange = (e) => {
    ...
    reader.onload = (event) => {
        ...
        imageBase64Data.value = event.target.result;
    };
...
const submitComment = async () => {
    if (!commentText.value.trim() && !imageBase64Data.value) return;
    isLoading.value = true;
    try {
        commentStore.sendComment(projectId, commentText.value, imageBase64Data.value);
        commentText.value = '';
        removeImage();
    } finally {
    ...
    <div class="flex flex-col lg:flex-row w-full h-screen bg-gray-100 overflow-hidden">
        <div class="flex-grow flex flex-col bg-white">
            ...
            <div class="flex-grow relative bg-slate-200">
                <iframe :src="`${config.public.auth.origin}/digital-content/${projectId}/${route.params.moduleName}/index.html`" class="absolute inset-0 w-full h-full border-none" />
            </div>
        </div>
            ...
            <div ref="scrollContainer" class="flex-grow overflow-y-auto p-4 space-y-4 bg-gray-50/50">
                <div v-for="item in commentStore.comments" :key="item._id" class="bg-white p-3 rounded-2xl border shadow-sm transition-all">
                    ...
                    <div v-if="item.image" class="mb-2 rounded-xl overflow-hidden border bg-slate-50">
                        <img :src="item.image" class="w-full h-auto max-h-64 object-contain cursor-zoom-in hover:opacity-90" @click="openPopup(item.image)" />
                    </div>
            ...
            <div class="p-4 border-t bg-white relative">
                ...
                <form @submit.prevent="submitComment" class="flex flex-col gap-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="submit" :disabled="isLoading || (!commentText.trim() && !imageBase64Data)" class="flex-grow bg-blue-600 text-white rounded-xl py-2.5 text-sm font-bold hover:bg-blue-700 disabled:opacity-40 transition-all shadow-md active:scale-95">
