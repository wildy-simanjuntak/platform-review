...
const { status, error, execute } = useLazyAsyncData(
  () => content.create(state.value),
  {
    immediate: false,
  },
);
...
const currentProject = computed(() =>
  project.items.find((v) => v._id === state.value.project),
);
const currentModule = computed(() =>
  module.items.find((v) => v._id === state.value.module),
);
        ...
        >
          <template #header>
            <div class="flex items-center justify-between">
              <div class="text-base">Create Content</div>
              <UButton
                color="gray"
                variant="ghost"
          ...
          <div class="flex flex-col space-y-[2rem]">
            ...
            <UFormGroup label="Name" name="name">
              ...
              >
                <template #label>
                  {{
                    nameOptions.find((opt) => opt.value === state.name)
                      ?.label || "Select..."
                  }}
                </template>
              </USelectMenu>
