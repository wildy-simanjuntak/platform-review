...
async function profile(body) {
  ...
  const res = await alovaInstance.Put(`/api/users/profile/${body._id}`, formData, { 
    headers: { Authorization: token.value }, 
    enableUpload: true 
  }).send();

  // 2. AMBIL data auth nuxt
  const { data: authData } = useAuth();

  // 3. PAKSA update data user di session
  // Ini akan memicu watcher di komponen Profile untuk jalan otomatis
  authData.value.user = { ...res }; 
