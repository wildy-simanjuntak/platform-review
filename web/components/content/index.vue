...
const items = computed(() => {
  return store.items.map((val) => {    
    return {
      ...val,
      module: val.module?.name || 'No Module', 
      project: val.module?.project?.name || 'Unknown Project',
    };
  });
});
...
const {
    ...
    error,
    execute,
} = useLazyAsyncData(() => Promise.all([store.getAll(), projectStore.getAll()]), {
    immediate: false,
});
            ...
            <div class="p-10 bg-white rounded-lg border-2">
                <div class="flex justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <UButton icon="i-solar-trash-bin-2-bold" size="sm" color="red" label="Remove" @click="showRemove = true" />
                    </div>
                </div>
