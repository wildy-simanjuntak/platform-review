...
const submit = async () => {
    if (!state.value.imageFile) {
        return toast.add({ title: 'Warning', description: 'Please select an image first', color: 'orange' });
    }
                ...
                <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
                    ...
                    <template #footer>
                        <div class="flex space-x-[1rem]">
                            ...
                            <UButton label="Save" :loading="status === 'pending'" @click="submit" class="text-white" />
                        </div>
                    </template>
                </UCard>
            </UForm>
