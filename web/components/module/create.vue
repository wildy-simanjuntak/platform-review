...
const programmerUsers = computed(() => {
  return users.items.filter((user) => {
    return user.role.name == "programmer";
  });
});
const close = () => {
        ...
        <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
          ...
          <div class="flex flex-col space-y-[2rem]">
            ...
            <UFormGroup label="Assignment" name="signTo">
              <USelectMenu 
                v-model="state.signTo" 
                :options="programmerUsers" 
                size="lg" 
                value-attribute="_id"
              ...
              >
                <template #label>
                  {{ currentSignTo?.username || 'Select Programmer...' }}
                </template>
              </USelectMenu>
            </UFormGroup>
          ...
          <template #footer>
            <div class="flex space-x-[1rem]">
              ...
              <UButton color="primary" label="Save" :loading="status === 'pending'" type="submit" class="text-white" />
            </div>
          </template>
        </UCard>
      </UForm>
