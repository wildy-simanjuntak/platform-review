...
export const useUsersStore = defineStore('users', () => {
  ...
  async function profile(body) {
    const { imageFile, _id } = body;
    const formData = new FormData();
    formData.append('imageFile', imageFile);

    const { token } = useAuth();

    const res = await alovaInstance.Put(`/api/users/profile/${_id}`, formData, {
    ...
    const index = this.items.findIndex((v) => v._id === res._id);
    if (index !== -1) {
      this.items[index] = { ...this.items[index], ...res };
    }
    return res;
  }
  ...
  async function updateBio(body) {
    console.log(body);
    const res = await alovaInstance.Put(`/api/users/update-bio/${body._id}`, body, { headers }).send();
    if (items.value && items.value.length > 0) {
      const index = items.value.findIndex((v) => v._id === res._id);
      if (index !== -1) {
        items.value[index] = { ...items.value[index], ...res };
      }
    }
  }

  return {
    items, avatar, getAll, create, update, remove, changePassword, profile, updateBio
  };
});